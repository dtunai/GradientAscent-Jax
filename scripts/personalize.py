...
def main(
    ...
    replacements = [
        ...
        ("gradient_ascent_jax", package_dir_name),
    ]
    if dry_run:
    ...
    # Rename 'gradient_ascent_jax' directory to `package_dir_name`.
    if not dry_run:
        (REPO_BASE / "gradient_ascent_jax").replace(REPO_BASE / package_dir_name)
    else:
        print(f"Renaming 'gradient_ascent_jax' directory to '{package_dir_name}'")
